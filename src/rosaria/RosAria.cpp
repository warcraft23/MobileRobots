...
class RosAriaNode
{
  ...
  protected:
    ...
    ros::WallTime veltime;
    ros::WallTime last_command_time_;

    std::string serial_port;
    std::string serial_port_kai;
...
/*
 * Initialize RosAriaNode
 * 用于设置ROS节点的一些参数，如订阅发布等，主要设置ROS的内容
*/
RosAriaNode::RosAriaNode(ros::NodeHandle nh) : myPublishCB(this, &RosAriaNode::publish), serial_port(""), serial_baud(0), use_sonar(false)
{
 ...
 //ROS_INFO( "RosAria: using port: [%s]", serial_port.c_str() );
  ...
  //cmdvel_sub = n.subscribe( "cmd_vel", 1, (boost::function <void(const geometry_msgs::TwistConstPtr&)>)boost::bind(&RosAriaNode::cmdvel_cb, this, _1 ));
    cmdvel_sub=n.subscribe("cmd_vel",1,&RosAriaNode::cmdvel_cb,this);
